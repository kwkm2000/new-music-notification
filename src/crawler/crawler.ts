...
export const crawl = async (cdType: "single" | "album") => {
  ...
  const url =
    cdType === "single"
      ? "https://www.oricon.co.jp/release/single/jp/"
      : "https://www.oricon.co.jp/release/album/";
  const jsonPath =
    cdType === "single"
      ? "./public/singleReleaseInfo.json"
      : "./public/albumReleaseInfo.json";
  ...
  for (let i = 0; i < PAGE_MAX; i++) {
    ...
    const result = await page.evaluate((selector) => {
      ...
      Array.from(sectionElements).forEach((sectionElement: HTMLElement) => {
        ...
        const formattedTitle = dateTitle
          .replace("年", "/")
          .replace("月", "/")
          .replace("日", "")
          .replace(" 発売", "");
        ...
        const items = Array.from(
          listElement.querySelectorAll(".jsc-block-ranking-detail")
        );

        releaseInfoObject[formattedTitle] = [];
        releaseInfoObject[formattedTitle] = items.map(
          (item: any): ReleaseInfo => {
            return {
